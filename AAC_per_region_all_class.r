...
# ---- #
# class treatment
dt3 <- dt2[!is.na(dt2$value), ]
# groupement en classe premier niveau
dt3$class <- NA

dt3$class[dt3$value %in% c("Forest", "Forest Wetland", "Forest Regenerating after Harvest <20 years", "Forest Regenerating after Fire <20 years", "Forest Wetland Regenerating after Harvest <20 years", "Forest Regenerating after Harvest 20-29 years", "Forest Wetland Regenerating after Harvest 20-29 years")] <- "Forestier"

dt3$class[dt3$value %in% c("Settlement", "Settlement Forest", "Vegetated Settlement", "High Reflectance Settlement", "Very High Reflectance Settlement", "Roads", "Newly-Detected Settlement <10 years", "Newly-Detected Settlement Forest <10 years", "Newly-Detected Vegetated Settlement <10 years", "Newly-Detected High Reflectance Settlement <10 years", "Newly-Detected Very High Reflectance Settlement <10 years", "Newly-Detected Road <10 years")] <- "Anthropique"
dt3$class[dt3$value %in% c("Cropland", "Annual Cropland", "Land Converted to Cropland", "Land Converted to Annual Cropland")] <- "Agricole"
dt3$class[dt3$value %in% c("Grassland Unmanaged", "Grassland Unmanaged")] <- "Grassland"
dt3$class[dt3$value %in% c("Wetland")] <- "Humide"
dt3$class[dt3$value %in% c("Water")] <- "Aquatique"
dt3$class[dt3$value %in% c("Snow and Ice", "Other Land")] <- "Other Land"

# groupement en classe deuxieme niveau
dt3$class2[dt3$class == "Anthropique"] <- "Anthropique"
dt3$class2[is.na(dt3$class2)] <- "Naturel"

df_colors <- data.frame(class = unique(dt3$class), color = c("brown2", "deepskyblue3", "darkolivegreen", "chocolate4", "darkgoldenrod", "aquamarine4", "navy"))
